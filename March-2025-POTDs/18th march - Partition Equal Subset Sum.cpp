...
public:
    ...
    bool solve(vector<int> &arr, int i, int sum, vector<vector<int>> &dp) {
        ...
        bool take = solve(arr, i+1, sum + arr[i], dp);    // Include current element
        bool not_take = solve(arr, i+1, sum, dp);         // Exclude current element
    ...
    bool equalPartition1(vector<int>& arr) {
        n = arr.size();
        int cumSum = accumulate(arr.begin(), arr.end(), 0);
    ...
    bool equalPartition2(vector<int>& arr) {
        n = arr.size();
        int cumSum = accumulate(arr.begin(), arr.end(), 0);
        ...
        for(int i = 1; i <= n; i++) {
            for(int j = 0; j <= target; j++) {
                if(j == 0) dp[i][j] = true;    // Empty subset gives sum 0
                    ...
                    dp[i][j] = dp[i-1][j-arr[i-1]] || dp[i-1][j];  // Take or not take
                }
    ...
    bool equalPartition3(vector<int>& arr) {
        n = arr.size();
        int cumSum = accumulate(arr.begin(), arr.end(), 0);
        if(cumSum % 2 == 1) return false;
        target = cumSum/2;
        ...
        for(int i = 1; i <= n; i++) {
            for(int j = 0; j <= target; j++) {
                if(j == 0) curr[j] = true;    // Empty subset gives sum 0
                    ...
                    curr[j] = prev[j-arr[i-1]] || prev[j];    // Take or not take
                }
    ...
    bool equalPartition4(vector<int>& arr) {
        n = arr.size();
        int cumSum = accumulate(arr.begin(), arr.end(), 0);
        if(cumSum % 2 == 1) return false;
        target = cumSum/2;
        ...
        for(int i = n-1; i >= 0; i--) {
            for(int j = target; j >= arr[i]; j--) {
                dp[j] = dp[j-arr[i]] || dp[j];    // Update if current element can contribute
            }
        }
        return dp[target];
    }
